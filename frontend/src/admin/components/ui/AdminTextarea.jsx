function AdminTextarea({ error, label, className = "", ...props }) {
  return (
    <label className={`block ${className}`}>
      ...
      <textarea
        className="min-h-32 w-full resize-y border border-brand-forest/15 bg-white px-4 py-3 text-sm text-brand-charcoal placeholder:text-brand-muted/65 focus:outline-none focus:ring-0"
        {...props}
      />
