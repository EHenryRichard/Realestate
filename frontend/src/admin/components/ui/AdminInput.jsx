function AdminInput({ error, label, className = "", ...props }) {
  return (
    <label className={`block min-w-0 w-full max-w-full ${className}`}>
      ...
      <input
        className="box-border min-h-12 min-w-0 w-full max-w-full border border-brand-forest/15 bg-white px-4 text-sm text-brand-charcoal placeholder:text-brand-muted/65 focus:outline-none focus:ring-0"
        {...props}
      />
